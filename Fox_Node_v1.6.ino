/*
  ...
  What it is:
    A tiny, configurable LoRa transmitter for RF direction-finding and geolocation drills.
    It’s built to be slippery enough to make teams argue about bearings, not menus.

  Party tricks:
    • Randomized burst timing (set min/max seconds).
    • Burst window with back-to-back packets (set on-air seconds).
    • Frequency playlist + hopping: OFF | SEQ | RANDOM, per-burst or per-packet.
    • Battery % via heltec_battery_percent().
    • TX LED blink you can flip on/off (setBlinkEnabled / toggleBlink).

  Tweakables:
    DEVICE_NAME, TX_MIN/MAX_INTERVAL_S, TX_ON_TIME_S, TX_IFG_MS, FREQ_LIST[],
    FREQ_HOP_ENABLED, FREQ_SELECT_MODE, HOP_EACH_PACKET,
    BANDWIDTH_KHZ / SPREADING_FACTOR / TRANSMIT_POWER_DBM,
    UPDATE_SENT_ON_TX, BATTERY_REFRESH_MS, and OLED contrast if you like it loud.

  Fine print (a.k.a. please don’t meet the FCC the hard way):
    • No duty-cycle nanny. You are the adult. Operate within your local rules!

  TL;DR:
    Turn on, hide it, and enjoy the hunt. You’re welcome.
*/
